...
# load dataset
dataframe = pd.read_csv("D:/WorkerCode/machinelearninginaction/Ch02/shenjingwangluo/iris.csv", header=None)
...
def baseline_model():
    model = Sequential()
    model.add(Dense(input_dim=4,activation='relu',units=10))
    model.add(Dropout(0.2))
    model.add(Dense(input_dim=10, activation='softmax',units=3))
    # Compile model
