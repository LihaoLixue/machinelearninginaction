...
def SVM_SMO_train(T, times, C, Mno, step):
    ...
    while time < times and updated:
        ...
        for i in range(len(gv.samples)):
            ...
            # agaist the KKT
            if (gv.samples[i].label[gv.cur_mno] * Ei < -T and ai < C) or (
                    gv.samples[i].label[gv.cur_mno] * Ei > T and ai > 0):
                for j in range(len(gv.samples)):
                    if j == i: continue
                    ...
                    eta = kii + kjj - 2 * kij
                    if eta <= 0: continue
                    new_aj = gv.models[gv.cur_mno].a[j] + gv.samples[j].label[gv.cur_mno] * (
                                gv.diff_dict[i] - gv.diff_dict[j]) / eta  # f 7.106
                    L = 0.0
                    H = 0.0
                    ...
                    if abs(a2_old - new_aj) < step:
                        print
                        "j = %d, is not moving enough" % j
                        continue

                    new_ai = a1_old + gv.samples[i].label[gv.cur_mno] * gv.samples[j].label[gv.cur_mno] * (
                                a2_old - new_aj)  # f 7.109
                    new_b1 = gv.models[gv.cur_mno].b - gv.diff_dict[i] - gv.samples[i].label[gv.cur_mno] * kii * (
                                new_ai - a1_old) - gv.samples[j].label[gv.cur_mno] * kji * (new_aj - a2_old)  # f7.115
                    new_b2 = gv.models[gv.cur_mno].b - gv.diff_dict[j] - gv.samples[i].label[gv.cur_mno] * kji * (
                                new_ai - a1_old) - gv.samples[j].label[gv.cur_mno] * kjj * (new_aj - a2_old)  # f7.116
