# coding:UTF-8
# base: https://www.cnblogs.com/Neo007/p/8460052.html#_caption_4
from numpy import *
...
def singleStump(dataArr, classLabels, D):
    ...
    minError = inf
    for i in list(range(n)):  # 对每一个特征
        # 取第i列特征的最小值和最大值，以确定步长
        rangeMin = dataMat[:, i].min()
        rangeMax = dataMat[:, i].max()
        stepSize = (rangeMax - rangeMin) / numSteps
        for j in list(range(-1, int(numSteps) + 1)):
...
def adaBoostTrain(dataArr, classLabels, G):
    weakClassArr = []
    m,n = shape(dataArr)  # 样本个数 ，row
    # 初始化D，即每个样本的权重均为1/n
    D = mat(ones((m, 1)) / n)
    aggClasEst = mat(zeros((m, 1)))

    for i in list(range(G)):  # G表示的是迭代次数
...
def adaBoostClassify(testData, weakClassify):
    ...
    for i in list(range(len(weakClassify))):  # weakClassify是一个列表
